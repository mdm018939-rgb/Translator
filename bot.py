...
def start(message):
    bot.send_message(
        ...
        "এই বটটি দিয়ে তুমি:\n\n"
        "🔤 *Translator* — যেকোনো লেখা যেকোনো ভাষায় অনুবাদ করতে পারবে। শুধু লেখাটা পাঠাও, তারপর কোন ভাষায় চাও বলো — ব্যস!\n\n"
...
def translator_get_text(message):
    ...
    msg = bot.send_message(chat_id, "🌐 কোন ভাষায় অনুবাদ করতে চাও?\n\n_(যেমন: বাংলা, English, French, Arabic)_", parse_mode="Markdown")
...
def translator_get_target_lang(message):
    ...
    try:
        ...
        # inline keyboard for copy
        markup = types.InlineKeyboardMarkup()
        markup.add(types.InlineKeyboardButton("📋 কপি করতে ক্লিক করো", callback_query_id="", switch_inline_query=translated))

        bot.send_message(chat_id, translated, reply_markup=markup)
    except Exception as e:
...
def language_detect(message):
    ...
    prompt = (
        ...
        "প্রথম লাইন: সেই ভাষার নিজস্ব লিপিতে ভাষার নাম (যেমন: English এর জন্য English, Arabic এর জন্য العربية, Hindi এর জন্য हिन्दी)\n"
        "দ্বিতীয় লাইন: বাংলায় ভাষার নাম (যেমন: ইংরেজি, আরবি, হিন্দি)\n"
    ...
    try:
        ...
        if len(lines) >= 2:
            ...
            bot.send_message(chat_id, f"🔎 *{native}* / {bangla}", parse_mode="Markdown", reply_markup=main_keyboard())
        else:
...
def fallback(message):
    bot.send_message(message.chat.id, "👇 নিচের বাটন থেকে অপশন বাছাই করো অথবা /start দাও।", reply_markup=main_keyboard())
