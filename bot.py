...
def typing_animation(chat_id, stop_event):
    ...
    dots = [".", "..", "..."]
    i = 0
    sent_msg = bot.send_message(chat_id, f"{text}.")
    time.sleep(0.4)
    while not stop_event.is_set():
        i = (i + 1) % 3
        try:
            bot.edit_message_text(f"{text}{dots[i]}", chat_id, sent_msg.message_id)
        except Exception:
            pass
        time.sleep(0.4)
    return sent_msg
...
def call_gemini_with_animation(chat_id, prompt, image_data=None, mime_type="image/jpeg"):
    ...
    def animate():
        ...
        while not stop_event.is_set():
            i = (i + 1) % 3
            try:
                bot.edit_message_text(f"{text}{dots[i]}", chat_id, sent.message_id)
            except Exception:
                pass
            time.sleep(0.4)
...
def main_keyboard():
    ...
    markup.add(types.KeyboardButton("🌐 CC Short Name"), types.KeyboardButton("🆔 Extract ID"))
...
def save_api_key(message):
    ...
    if message.text in ["⚙️ Settings", "🔙 Back", "🌍 Language", "🔤 Translator", "🌐 CC Short Name", "🆔 Extract ID"]:
...
MENU_BUTTONS = ["⚙️ Settings", "🔙 Back", "🌍 Language", "🔤 Translator", "🌐 CC Short Name", "🆔 Extract ID",
...
def translator_get_target_lang(message):
    ...
    bot.send_message(chat_id, "✅ অনুবাদ সম্পন্ন!", reply_markup=main_keyboard())
    user_data.pop(chat_id, None)
...
# ─── Extract ID ───
@bot.message_handler(func=lambda m: m.text == "🆔 Extract ID")
def extract_id_start(message):
    cancel_previous(message.chat.id)
    msg = bot.send_message(
        message.chat.id,
        "🆔 Send Facebook links or cookies directly (one item per line)."
    )
