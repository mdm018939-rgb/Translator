...
def call_gemini_with_animation(chat_id, prompt, image_data=None, mime_type="image/jpeg"):
    ...
    # Typing message delete করো
    try:
        bot.delete_message(chat_id, anim_msg[0].message_id)
    except Exception:
        pass
...
def save_api_key(message):
    ...
    if len(key) < 10:
        bot.send_message(chat_id, "⚠️ এটা সঠিক API Key না।", reply_markup=settings_keyboard())
        return
    ...
    try:
        ...
        if "403" in err or "401" in err or "invalid" in err.lower() or "api key" in err.lower():
            bot.send_message(chat_id, "❌ ভুল API Key! সঠিক Key দাও:\n👉 https://aistudio.google.com", reply_markup=settings_keyboard())
        else:
            bot.send_message(chat_id, "⚠️ Key যাচাই করতে পারিনি।", reply_markup=settings_keyboard())
...
MENU_BUTTONS = ["⚙️ Settings", "🔙 Back", "🌍 Language", "🔤 Translator", "🌐 CC Short Name", "🆔 Extract UID",
                "🔑 Add API Key", "❌ Remove API Key", "📊 আমার ব্যবহার", "❓ Help"]
...
def translator_get_target_lang(message):
    ...
    if result_holder["error"]:
        ...
        if "RATE_LIMIT" in str(e):
            bot.send_message(chat_id, "⏳ এই মুহূর্তে একটু ব্যস্ত আছি!\n\nদয়া করে মাত্র ১ মিনিট পরে আবার চেষ্টা করুন 🙏", reply_markup=main_keyboard())
        else:
    ...
    if "INVALID_LANGUAGE" in translated:
        msg = bot.send_message(chat_id, "⚠️ এটি কোনো ভাষার নাম নয়!\n\n🌐 ভাষার নাম লেখো:")
        bot.register_next_step_handler(msg, translator_get_target_lang)
        return
