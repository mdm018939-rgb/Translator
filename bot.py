...
def save_api_key(message):
    ...
    if len(key) < 10:
        bot.send_message(chat_id, "⚠️ এটা সঠিক API Key না।", reply_markup=settings_keyboard(chat_id))
        return
    ...
    try:
        ...
        if "403" in err or "401" in err or "invalid" in err.lower() or "api key" in err.lower():
            bot.send_message(chat_id, "❌ ভুল API Key! সঠিক Key দাও:\n👉 https://aistudio.google.com", reply_markup=settings_keyboard(chat_id))
        else:
            bot.send_message(chat_id, "⚠️ Key যাচাই করতে পারিনি।", reply_markup=settings_keyboard(chat_id))
...
def remove_api_key(message):
    ...
    user = get_user(chat_id)
    if not user["api_key"]:
        bot.send_message(chat_id, "⚠️ তোমার কোনো API Key সেভ নেই।", reply_markup=settings_keyboard(chat_id))
        return
    user["api_key"] = None
    bot.send_message(chat_id, "✅ API Key রিমুভ হয়েছে।", reply_markup=settings_keyboard(chat_id))
...
def my_usage(message):
    ...
    today = datetime.date.today().isoformat()
    if user["api_key"]:
        bot.send_message(chat_id, "✅ তোমার নিজের API Key আছে — *আনলিমিটেড* ব্যবহার করতে পারবে!", parse_mode="Markdown", reply_markup=settings_keyboard(chat_id))
    else:
        if user["date"] != today:
            used = 0
        else:
...
def translator_get_target_lang(message):
    ...
    if result_holder["error"]:
        ...
        if "RATE_LIMIT" in str(e):
            bot.edit_message_text("⏳ এই মুহূর্তে একটু ব্যস্ত আছি!\n\nদয়া করে মাত্র ১ মিনিট পরে আবার চেষ্টা করুন 🙏", chat_id, anim_msg.message_id)
        else:
    ...
    if "INVALID_LANGUAGE" in translated:
        bot.edit_message_text("⚠️ এটি কোনো ভাষার নাম নয়!\n\n🌐 ভাষার নাম লেখো:", chat_id, anim_msg.message_id)
        bot.register_next_step_handler(anim_msg, translator_get_target_lang)
        return
...
def language_detect(message):
    ...
    if len(lines) >= 2:
        ...
        bot.edit_message_text(f"🔎 *{native}* / {bangla}", chat_id, anim_msg.message_id, parse_mode="Markdown")
    else:
